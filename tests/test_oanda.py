...
from oandabase.oanda import Oanda, Order, Account, DataFeed
...
class TestOanda:
    ...
    def test_set_base_url(self):
        if oanda.practice:
            message = ("Practice account url not set correctly"
                       f" - {oanda.base_url}")
            assert oanda.base_url == \
                'https://api-fxpractice.oanda.com', message

        elif not oanda_live.practice:
            message = ("Live account url not set correctly"
                       f" - {oanda_live.base_url}")
            assert oanda_live.base_url ==\
                'https://stream-fxtrade.oanda.com', message
    ...
    def test_pair_switching(self):
        message = ("Pair should have changed to 'GBP_USD'"
                   f" - It's {oanda_gbp.pair}")
        assert oanda_gbp.pair == 'GBP_USD', message

    def test_set_headers(self):
        message = ("Authorizarion Header not set correctly,"
                   " it does not contain the API token!")
...
class TestAccount():
    ...
    def test_get_account(self):
        ...
        message3 = ("The account info returned is not for the "
                    f"correct account! - acc:{oanda.account}"
                    " - returned{account_id}")
        assert oanda.account == account_id, message3

    def test_account_balance(self):
        message = ("There is no money in the account!! "
                   f"- balance returned = {acc.balance}")
...
class TestDataFeed():
    ...
    def test_data0(self):
        if data.backfill:
            ...
            message = ("We should have got 500 candle back "
                       f"- got {data_length} instead")
            assert data_length == 500, message

        message2 = "We did not get a list back fro data0 as expected"
